...
export default function Layout({ children }) {
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: theme.palette.background.default }}>
      <AppBar position="sticky" color="default">
        <Toolbar>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexGrow: 1, cursor: 'pointer' }} onClick={() => navigate('/') }>
            <Avatar src="/logo.png" alt="MPSP Projetos" variant="square" sx={{ width: 28, height: 28, borderRadius: 0 }} />
            <Typography variant="h6" sx={{ fontWeight: 800, color: theme.palette.text.primary }}>MPSP Projetos</Typography>
          </Box>
          {isSmall ? (
            <>
              ...
              <Menu anchorEl={menuEl} open={Boolean(menuEl)} onClose={closeMenu} keepMounted>
                ...
                <MenuItem onClick={closeMenu} component={Link} to="/projects" selected={active.projects}>Projetos</MenuItem>
                <MenuItem onClick={closeMenu} component={Link} to="/my-projects" selected={active.myProjects}>Meus Projetos</MenuItem>
                <Divider/>
            ...
            <>
              ...
              <Button color="primary" variant={active.projects ? 'contained' : 'text'} component={Link} to="/projects">Projetos</Button>
              <Button color="primary" variant={active.myProjects ? 'contained' : 'text'} component={Link} to="/my-projects">Meus Projetos</Button>
      ...
      <Container maxWidth={false} sx={{ py: { xs: 2, md: 3 }, px: { xs: 2, md: 6 } }}>
        {children}
      </Container>
    </Box>
  )
